    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    ui->tableWidget_client->setSelectionBehavior(QAbstractItemView::SelectRows);//this
    ui->tableWidget_client->setSelectionMode(QAbstractItemView::SingleSelection);//this
...
void MainWindow::afficherListeClients()
{
    ...
    for (const Client &c : liste) {
        ...
        ui->tableWidget_client->setItem(row, 3, new QTableWidgetItem(c.cin())); // ✅ CIN added
...
void MainWindow::afficherWidgetAjoutClient()
{
    ...
    connect(ajouter, &QPushButton::clicked, this, [=]() {
        ...
        QString typeText    = type->currentText().trimmed();
        QString adresseText = adresse->text().trimmed();

        if (nomText.isEmpty() || prenomText.isEmpty() || telText.isEmpty() ||
            cinText.isEmpty() || zoneText.isEmpty() || adresseText.isEmpty()) {
            showMessage(dialog, "Champs manquants",
                        "Merci de remplir tous les champs obligatoires.", "warning");
            return;
        }
        ...
        if (!regexNomPrenom.match(nomText).hasMatch() ||
            ...
            showMessage(dialog, "Nom/Prénom invalide",
                        "Le nom et le prénom doivent contenir uniquement des lettres.", "error");
            return;
        }

        QRegularExpression regexCIN("^\\d{8}$");
        if (!regexCIN.match(cinText).hasMatch()) {
            showMessage(dialog, "CIN invalide",
                        "Le CIN doit contenir exactement 8 chiffres.", "error");
            return;
        }

        QRegularExpression regexTel("^\\d{8}$");
        if (!regexTel.match(telText).hasMatch()) {
            showMessage(dialog, "Téléphone invalide",
                        "Le numéro de téléphone doit contenir exactement 8 chiffres.", "error");
            return;
        }
...
void MainWindow::on_bouttonmodifierclient_clicked()
{
    ...
    if (row < 0) {
        showMessage(this, "Attention",
                    "⚠️ Veuillez sélectionner un client dans la liste avant de modifier !",
                    "warning");
        return;
    }
    ...
    QString idText = getItemText(0);
    if (idText.isEmpty()) {
        showMessage(this, "Erreur",
                    "Impossible de récupérer l'identifiant du client.", "error");
        return;
    }

    Client c;
    c.setIdClient(idText.toInt());
...
void MainWindow::afficherWidgetModifierClient(Client client)
{
    ...
    nom->setValidator(new QRegularExpressionValidator(rxLetters, nom));
    prenom->setValidator(new QRegularExpressionValidator(rxLetters, prenom));
    QRegularExpression rxNumbers("^[0-9]+$");
    cin->setValidator(new QRegularExpressionValidator(rxNumbers, cin));
    telephone->setValidator(new QRegularExpressionValidator(rxNumbers, telephone));
    cin->setMaxLength(8);
    telephone->setMaxLength(8);
    ...
    connect(modifier, &QPushButton::clicked, this, [=]() {
        if (cin->text().length() != 8 || telephone->text().length() != 8) {
            showMessage(dialog, "Erreur",
                        "CIN et Téléphone doivent contenir exactement 8 chiffres.", "error");
            return;
        }
        ...
        c.setTypeClient(type->currentText().trimmed());
...
void MainWindow::on_bouttonsupprimerclient_clicked()
{
    ...
    if (!idItem || idItem->text().trimmed().isEmpty()) {
        showMessage(this, "Erreur",
                    "Impossible de récupérer l'identifiant du client sélectionné.", "error");
        return;
    }
    ...
    // ── Dialog de confirmation custom (même style que les autres modules)
...
void MainWindow::on_bouttonrechercheemp_clicked()
{
    ...
    EmployeRecherche rec;
    QSqlQueryModel *model = rec.rechercher(recherche);
...
{
    EmployeExporter exporter;
    exporter.exporterPDF(this);
}
// ================================================================
// === MODULE EMPLOYE — TRI =======================================
// ================================================================
...
void MainWindow::on_trieemp_currentIndexChanged(int index)
{
    ...
    EmployeTri tri;
    QSqlQueryModel *model = tri.trierPar(colonnes[index]);
...
{
    EmployeStat stat;
    stat.afficherStatistiques(this);
}
