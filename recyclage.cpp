...
void Recyclage::afficherStatistiques(QWidget *parent)
{
    ...
    // ── Comptage par type de matériau ────────────────────────────
    auto countType = [&](const QString &type) -> int {
        q.prepare("SELECT COUNT(*) FROM RECYCLAGES WHERE UPPER(Type_materiau)=UPPER(:t)");
        q.bindValue(":t", type);
        if (q.exec() && q.next()) return q.value(0).toInt();
        return 0;
    };

    int plastique = countType("Plastique");
    int metal     = countType("Métal");
    int verre     = countType("Verre");
    int papier    = countType("Papier");
    int organique = countType("Organique");

    // Autres types non couverts
    q.prepare(
        "SELECT COUNT(*) FROM RECYCLAGES WHERE Type_materiau IS NOT NULL AND "
        "UPPER(Type_materiau) NOT IN "
        "('PLASTIQUE','MÉTAL','VERRE','PAPIER','ORGANIQUE')"
        );
    int autre = (q.exec() && q.next()) ? q.value(0).toInt() : 0;

    int total = plastique + metal + verre + papier + organique + autre;
    if (total == 0) total = 1;
    ...
    // ── Titre ────────────────────────────────────────────────────
    QLabel *title = new QLabel("📊 Répartition des Recyclages par Type de Matériau");
    ...
    QPieSlice *s1 = series->append("Plastique",  plastique);
    QPieSlice *s2 = series->append("Métal",      metal);
    QPieSlice *s3 = series->append("Verre",      verre);
    QPieSlice *s4 = series->append("Papier",     papier);
    QPieSlice *s5 = series->append("Organique",  organique);
    QPieSlice *s6 = series->append("Autre",      autre);

    s1->setBrush(QColor("#6366F1"));   // violet  — Plastique
    s2->setBrush(QColor("#14B8A6"));   // cyan    — Métal
    s3->setBrush(QColor("#3B82F6"));   // bleu    — Verre
    s4->setBrush(QColor("#F59E0B"));   // jaune   — Papier
    s5->setBrush(QColor("#10B981"));   // vert    — Organique
    s6->setBrush(QColor("#9CA3AF"));   // gris    — Autre

    s1->setLabel(QString("Plastique %1%") .arg((plastique * 100.0) / total, 0, 'f', 1));
    s2->setLabel(QString("Métal %1%")     .arg((metal     * 100.0) / total, 0, 'f', 1));
    s3->setLabel(QString("Verre %1%")     .arg((verre     * 100.0) / total, 0, 'f', 1));
    s4->setLabel(QString("Papier %1%")    .arg((papier    * 100.0) / total, 0, 'f', 1));
    s5->setLabel(QString("Organique %1%") .arg((organique * 100.0) / total, 0, 'f', 1));
    s6->setLabel(QString("Autre %1%")     .arg((autre     * 100.0) / total, 0, 'f', 1));
    ...
    QObject::connect(s6, &QPieSlice::hovered, [=](bool state){ s6->setExploded(state); });

    // ── Chart ────────────────────────────────────────────────────
    QChart *chart = new QChart();
    chart->addSeries(series);
    chart->setTitle("Répartition par type de matériau");
